...
Esta API RESTful maneja la lógica de negocio completa de un sistema de restaurante, desde la gestión del menú hasta el procesamiento transaccional de pedidos y reportes administrativos inteligentes.
...
-   **Bloqueo de Seguridad:** Las órdenes pagadas (`paid`) se bloquean automáticamente contra ediciones.
...
Endpoints dedicados para que cada usuario gestione su cuenta:
...
El endpoint `/api/dashboard` adapta su respuesta según quién lo consulta:

-   **Admin:** Ve métricas globales, ventas totales, ranking de meseros y gráficos completos.
-   **Mesero:** Ve solo **sus** ventas del día y gráficos de su rendimiento personal.
-   **Cocina:** Ve solo conteo de pedidos y productos populares para preparación (sin datos financieros).

#### 📈 Gráficos Avanzados

-   **Sales Chart:** Generación de arrays para Chart.js con relleno inteligente de días vacíos (últimos 7 días).
-   **Category Chart:** Distribución de productos vendidos por categoría.
...
| POST           | `/api/profile/update`     | Actualizar mis datos/foto           | ✅   |
| PUT            | `/api/profile/password`   | Cambiar mi contraseña               | ✅   |
| **Categorías** |
...
| PUT            | `/api/orders/{id}`        | Actualizar pedido (Completar/Pagar) | ✅   |
| PATCH          | `/api/orders/{id}/status` | Cambiar estado (cocina/mesero)      | ✅   |
| **Dashboard**  |
| GET            | `/api/dashboard`          | Reportes y estadísticas (Por Rol)   | ✅   |
